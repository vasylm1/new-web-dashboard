...
def run(lang):
    ...
    with left:
        ...
        full_name = st.text_input(t["full_name"], placeholder="Vasyl Madei")
        job_title = st.text_input(t["job_title"], placeholder="Product Marketeer")
        company = st.text_input(t["company_name"], placeholder="Signify")

        st.markdown(f"**{t['contact_info']}**")
        phone = st.text_input(t["phone_number"], placeholder="+48 ...")
        ...
        socials = []
        for label, emoji, placeholder in SOCIALS:
            url = st.text_input(f"{emoji} {label}", placeholder=placeholder, key=f"sig_{label}")
